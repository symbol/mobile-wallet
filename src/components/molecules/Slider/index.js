...
import { FlatList, View, Text, TouchableOpacity, Platform, I18nManager } from 'react-native';
...
class Slider extends Component<Props, State> {
    ...
    return (
      <View style={{ width, height }}>
        {renderItem ? (
          ...
          <Text style={styles.errorText}>renderItem() not implemented</Text>
        )}
      </View>
    );
  };
  ...
  renderDefaultButton = (name: string) => {
    ...
    if (bottomButton) {
      const defaultButtonStyle =
        name === 'Skip' || name === 'Prev' ? { backgroundColor: 'transparent' } : null;
      content = (
        <View style={[styles.bottomButton, defaultButtonStyle, buttonStyle]}>{content}</View>
      );
    }
  ...
  renderPagination = () => {
    ...
    const skipBtn =
      (!isFirstSlide && this.renderPrevButton()) || (!isLastSlide && this.renderSkipButton());
          ...
          {slides.length > 1 &&
            slides.map((_, i) => (
              <TouchableOpacity
                ...
                style={[
                  styles.dot,
                  this.rtlSafeIndex(i) === activeIndex ? activeDotStyle : dotStyle,
                ]}
