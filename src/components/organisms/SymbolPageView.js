...
const WizardStepView = (props: Props) => {
		...
		if(theme === 'light')
			return (
				...
				>
					...
					{!isLoading && isFade && <FadeView style={styles.container} duration={300}>
						{ props.children }
					</FadeView>}
	...
	const footerStyle = buttons && buttons.length === 1 && !footer
			...
			<View style={ styles.pageContainer }>
				...
				{!noScroll && <ScrollView style={styles.scrollView} persistentScrollbar={true}>
					...
					<View style={ styles.content } duration={100}>
						...
						{/* Error message */}
						{ isError && <View style={{flex: 1, alignItems: 'center'}}><Text style={ [styles.error, styles.center] }>{ errorMessage }</Text></View> }
					</View>
				</ScrollView>}
				...
				{(footerTitle || buttons || footer || (isError && errorButtons)) && <View style={footerStyle}>
					{/* Footer title */}
					{ !isLoading && !isError && footerTitle && <Text style={styles.footerTitle}>{ footerTitle }</Text> }

					{/* Buttons */}
					{ !isLoading && !isError && buttons && buttons.length === 1 && <Button
					...
					{ !isLoading && isError && errorButtons && errorButtons.length === 1 && <Button
						style={ styles.buttons }
