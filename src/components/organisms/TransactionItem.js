...
import { Col, Icon, Row, TableView, Text, Trunc } from '@src/components';
            ...
            case 'nativeMosaicIncoming':
                return (
                    <Text
                        type="bold"
                        theme="light"
                        style={[styles.amountIncoming, { marginLeft: 5 }]}
                    >
                        {value.value}
                    </Text>
                );
            case 'nativeMosaicOutgoing':
                return (
                    <Text
                        type="bold"
                        theme="light"
                        style={[styles.amounteOutgoing, { marginLeft: 5 }]}
                    >
                        {'-' + value.value}
                    </Text>
                );
    ...
    render = () => {
        ...
        switch (transaction.type) {
            case 'transfer': // TODO: replace with SDK.TransactionType.TRANSFER
                const transferType =
                    transaction.recipientAddress === currentAddress
                        ? 'incoming'
                        : 'outgoing';
                const currencyMosaic = filterCurrencyMosaic(
                    transaction.mosaics,
                    network
                );
                iconName =
                ...
                if (!currencyMosaic) {
                    ...
                    if (currencyMosaic && transferType === 'incoming')
                        values.push({
                            ...
                            value:
                                currencyMosaic.amount /
                                Math.pow(10, currencyMosaic.divisibility),
                        });
                ...
                if (
                    (transaction.mosaics.length > 1 && currencyMosaic) ||
                    (transaction.mosaics.length > 0 && !currencyMosaic)
                ) {
                ...
                break;
            case 'aggregate':
                const isPostLaunchOptIn =
                    getFinanceBotPublicKeys(network.type).indexOf(
                        transaction.signTransactionObject.signer.publicKey
                    ) >= 0;
                let postLaunchOptinAmount = '';

                if (this.state) {
                    const transaction: SdkAggregateTransaction = this.state
                        .fullTransaction;
                    ...
                    mosaics.forEach(
                        mosaic => (sumAmount = sumAmount.add(mosaic.amount))
                    );
                    postLaunchOptinAmount =
                        sumAmount.compact() / Math.pow(10, 6);
                }

                iconName = isPostLaunchOptIn
                    ? 'postLaunchOptIn'
                    : transaction.type;
