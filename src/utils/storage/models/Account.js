...
class Account extends Realm.Object {
  ...
  get currencyBalance() {
    ...
    const divisibility =
      networkCurrency && networkCurrency.mosaic ? 10 ** networkCurrency.mosaic.divisibility : 1;
...
const createAccount = async (
  ...
  return from(
    realmInstance.then(realm => {
      try {
        const accountLength = realm.objects('Account').filtered(`networkType == ${networkType}`)
          .length;
        realm.write(() => {
        ...
        try {
          accounts.forEach(account => {
            ...
            tempAccount.linkedAccount =
              account.accountType === (AccountType.Main || AccountType.Remote);
